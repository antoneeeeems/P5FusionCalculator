...
class FusionCalculator {
    ...
    private customPersonaList: PersonaData[];
    private personaMap: {[key: string]: PersonaData};
    private special2Combos: any[];
    private specialCombos: any[];
    private arcana2Combos: any[];
    private rareCombos: any;
    private rarePersonae: string[];
    private getResultArcana: (arcana1: string, arcana2: string) => string;

    constructor(
        personaeByArcana: {[arcana: string]: PersonaData[]},
        customPersonaList: PersonaData[],
        personaMap: {[key: string]: PersonaData},
        special2Combos: any[],
        specialCombos: any[],
        arcana2Combos: any[],
        rareCombos: any,
        rarePersonae: string[],
        getResultArcana: (arcana1: string, arcana2: string) => string
    ) {
        this.personaeByArcana = personaeByArcana;
        this.customPersonaList = customPersonaList;
        this.personaMap = personaMap;
        this.special2Combos = special2Combos;
        this.specialCombos = specialCombos;
        this.arcana2Combos = arcana2Combos;
        this.rareCombos = rareCombos;
        this.rarePersonae = rarePersonae;
        this.getResultArcana = getResultArcana;
    }
    ...
    public getAllResultingRecipesFrom(persona: PersonaData): Recipe[] {
        ...
        for (let i = 0; i < this.customPersonaList.length; i++) {
            let result = this.fuse(persona, this.customPersonaList[i]);
            if (result !== null) {
                let recipe = {
                    sources: [persona, this.customPersonaList[i]],
                    result: result
                };
    ...
    private getSpecialFuseResult(persona1: PersonaData, persona2: PersonaData): PersonaData {
        for (let x = 0; x < this.special2Combos.length; x++) {
            let combo = this.special2Combos[x];
            if (((persona1.name === combo.sources[0] && persona2.name === combo.sources[1]) ||
                ...
                return this.personaMap[combo.result];
            }
        }
    ...
    private fuseNormal(persona1: PersonaData, persona2: PersonaData): PersonaData {
        ...
        let arcana = this.getResultArcana(persona1.arcana, persona2.arcana);
        if (!arcana) {
    ...
    private fuseRare(rarePersona: PersonaData, mainPersona: PersonaData): PersonaData {
        let modifier = this.rareCombos[mainPersona.arcana][this.rarePersonae.indexOf(rarePersona.name)];
    ...
    private getSpecialRecipe(persona: PersonaData): Recipe[] {
        ...
        for (let i = 0; i < this.specialCombos.length; i++) {
            ...
            if (persona.name === combo.result) {
                let recipe = {
                    sources: [],
                    result: this.personaMap[combo.result]
                };
                for (let j = 0; j < combo.sources.length ; j++) {
                    recipe.sources.push(this.personaMap[combo.sources[j]]);
                }
    ...
    private getArcanaRecipes(arcana: string): Recipe[] {
        ...
        for (let i = 0; i < this.rarePersonae.length; i++) {
            let rarePersona = this.personaMap[this.rarePersonae[i]];
            let personae = this.personaeByArcana[arcana];
